...
from .correlation_jit import (  # , correlation_for_params
    build_corr_context_jit,
    correlation_for_params_jit,
)
...
def _corr_worker_loop(
    ...
    def _load_payload(payload_id: str) -> bool:
        ...
        if (
            payload_id == current_payload_id
            and ref_img is not None
            and ctx_jit is not None
        ):
            return True
        ...
        # warm-up compile
        _ = correlation_for_params_jit(
            ref_img, target_img, 0.0, 0.0, 1.0, 1.0, center=center, ctx=ctx_jit
        )
        return True
...
def optimize_shift_and_scale(
    ...
    if task_queue is None or result_queue is None or shared_payloads is None:
        raise ValueError(
            "task_queue, result_queue, and shared_payloads must be provided"
        )
    ...
    if not is_4d:
        ...
        # Initialize
        current_center = clamp_point(
            (int(round(dx0)), int(round(dy0))), shift_x, shift_y
        )
        ...
        while True:
            ...
            iteration_number += 1
            msg = (
                f"Iteration {iteration_number}: current center {current_center}, "
                f"cache size {len(cache)}/{total_points}, phase {search_phase}..."
            )
            _vprint(verbose, 2, msg)
            ...
            local_points = [
                (current_center[0], current_center[1], cache[current_center])
            ]
            ...
            if best_corr > global_best_corr:
                global_best_corr = best_corr
            msg = (
                f"Best local corr: {cache[current_center]:.5f}, "
                f"best overall corr: {global_best_corr:.5f} at shift {best_point}."
            )
            _vprint(verbose, 2, msg)
            ...
            if phase_switched and not force_plateau:
                _vprint(
                    verbose, 2, f"Switching to plateau phase at corr={best_corr:.5f}"
                )
            if plateau_met:
            ...
            candidate_center = clamp_point(
                (
                    ...
                    current_center[1] + int(step_vec[1]),
                ),
                shift_x,
                shift_y,
            )
            ...
            while candidate_center in cache and candidate_center != current_center:
                ...
                if stall_counts[candidate_center] >= 2:
                    fallback_local = first_unvisited(
                        candidate_center, all_points, cache
                    )
                ...
                next_candidate = clamp_point(
                    (
                        candidate_center[0] + int(step_vec[0]),
                        candidate_center[1] + int(step_vec[1]),
                    ),
                    shift_x,
                    shift_y,
                )
                ...
                if candidate_center == current_center or not point_in_bounds(
                    candidate_center, shift_x, shift_y
                ):
                    break

            if (
                candidate_center == current_center
                or not point_in_bounds(candidate_center, shift_x, shift_y)
        ...
        _vprint(
            verbose,
            1,
            f"Best correlation found: {best_corr:.6f} at shift ({best_point[0]}, {best_point[1]}).",
        )
        return (
            {
        ...
        _vprint(
            verbose,
            2,
            f"4D search: {len(dx_vals)}×{len(dy_vals)}×{len(sx_vals)}×{len(sy_vals)} = {total_points} points",
        )
        _vprint(verbose, 2, f"Using {n_workers} workers with batch processing")
        ...
        def _record_corr_4d(
            dx: int, dy: int, sx: float, sy: float, corr_val: float
        ) -> None:
        ...
        current_center_4d = (
            ...
            (
                float(sx0) if sx0 is not None else 1.0,
                float(sy0) if sy0 is not None else 1.0,
            ),
        )
        ...
        while True:
            ...
            iteration_number += 1
            msg = (
                f"Iteration {iteration_number}: center {current_center_4d}, "
                f"cache {len(cache4d)}/{total_points}, phase {search_phase}"
            )
            _vprint(verbose, 2, msg)
            ...
            msg = f"Best corr: {best_corr:.5f} at {best_key}"
            _vprint(verbose, 2, msg)
        ...
        # Workers remain alive for subsequent calls (managed externally)
        msg = (
            f"Best correlation: {best_corr:.6f} at shift=({best_key[0]}, {best_key[1]}), "
            f"scale=({best_key[2]:.3f}, {best_key[3]:.3f})"
        )
