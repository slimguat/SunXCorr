...
def build_shift_structures(
    ...
    offsets.sort(
        key=lambda xy: (xy[0] * xy[0] + xy[1] * xy[1], abs(xy[0]) + abs(xy[1]))
    )
...
def clamp_point_nd(
    point: Tuple[int, ...], axis_ranges: Sequence[int]
) -> Tuple[int, ...]:
    ...
    """
    return tuple(
        int(np.clip(coord, -rng, rng)) for coord, rng in zip(point, axis_ranges)
    )
...
def compute_gradient_step_nd(
    ...
    if best_point != current_center:
        return tuple(
            float(best_point[idx] - current_center[idx])
            for idx in range(len(current_center))
        )
...
def update_phase_state(
    ...
    elif last_best_increase < max_corr_goal:
        plateau_counter = 0
    else:
        plateau_counter += 1
        if phase == 1:
