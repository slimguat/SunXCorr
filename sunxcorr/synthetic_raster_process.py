...
class SyntheticRasterProcess(CoalignmentNode):
    ...
    def _execute_own_process(self, working_map: GenericMap) -> None:
        ...
        _vprint(
            verbose, 2, f"  Reference sequence: {len(reference_sequence)} FSI images"
        )
        ...
        _vprint(
            verbose,
            1,
            f"Building synthetic raster from {len(reference_sequence)} FSI images...",
        )
        ...
        _vprint(
            verbose,
            2,
            f"  Max shift (pixels): X={max_shift_pixels_x}, Y={max_shift_pixels_y}",
        )
        ...
        shift_x_arcsec, shift_y_arcsec = pixels_to_arcsec(
            (best_params["dx"], best_params["dy"]), working_map
        )
        ...
        corrected_map = apply_shift_and_scale_to_map(
            working_map, shift_x_arcsec, shift_y_arcsec, scale_x, scale_y
        )
