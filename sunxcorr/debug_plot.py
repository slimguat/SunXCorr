...
class DebugPlotContext:
    ...
    def render_comparison_animation(
        ...
        im2_corr = ax_after.pcolormesh(
            ...
            norm=normit(corrected_map.data),
            rasterized=True,
        )
        ...
        gif_path = (
            ...
            f"comparison_animation{date}_{phase_suffix}.gif"
        )
        ...
        *,
        ...
        if ref_map is not None and target_map is not None and corrected_map is not None:
            ...
            contour_levels = np.percentile(
                tgt_data[np.isfinite(tgt_data)], [70, 80, 90, 95, 99]
            )
            ...
            overlay_fig, axes = plt.subplots(
                2, 2, figsize=(10, 10), sharex=False, sharey=False
            )
        ...
        contour_levels = np.percentile(
            target_img[np.isfinite(target_img)], [70, 80, 90, 95, 99]
        )
        ...
        overlay_ax.set_title(
            f"Reference vs shifted target (dx={shift_dx}, dy={shift_dy})"
        )
    ...
    def render_history_plot(
        ...
        if scale_varies:
            ...
            sc2 = axes[1].scatter(
                sx, sy, c=corr, cmap="magma", s=22, alpha=0.8)
            ...
            # dx vs dy colored by corr
            sc1 = axes[0].scatter(
                dx, dy, c=corr, cmap="magma", s=22, alpha=0.8)
            axes[0].set_xlabel("dx (pixels)")
            axes[0].set_ylabel("dy (pixels)")
            axes[0].set_title("dx vs dy (color=corr)")
            axes[0].grid(True, alpha=0.3)
...
def create_debug_context(
    ...
    *,
    ...
    if pdf_writer is None:
        ...
        if pdf_path is None:
            raise ValueError(
                "pdf_path must be provided when reusing a PdfPages writer."
            )
