...
class SingleMapProcess(CoalignmentNode):
    ...
    def __init__(
        ...
        # Set node identity
        bin_label = f"{self.bin_kernel.value:.1f}as" if self.bin_kernel.value > 1 else "full-res"
    ...
    def _execute_own_process(self, working_map: GenericMap) -> None:
        ...
        _vprint(verbose, 2, f"  Max shift (pixels, unbinned): X={max_shift_pixels_x}, Y={max_shift_pixels_y}")
        ...
        if self.bin_kernel.value > 1:
            ...
            max_shift_binned_pixels = (max_shift_binned_pixels_x, max_shift_binned_pixels_y)
            _vprint(verbose, 2, f"  Max shift (pixels, binned): X={max_shift_binned_pixels_x}, Y={max_shift_binned_pixels_y}")
        else:
            _vprint(verbose, 1, "Running at full resolution (no binning)")
            working_binned = working_map
        ...
        # Get binned map pixel scale for shift conversion
        # binned_pixel_scale = get_pixel_scale_quantity(working_binned)
        
        # Get center pixel from binned map CRPIX
        center_pix = (
            float(working_binned.meta.get('CRPIX1', working_binned.data.shape[1] / 2.0)),
            float(working_binned.meta.get('CRPIX2', working_binned.data.shape[0] / 2.0))
        )
        ...
        binned_pixel_scale_x, binned_pixel_scale_y = get_pixel_scale_quantity(working_binned)
        ...
        corrected_map = apply_shift_to_map(working_map, shift_x_arcsec, shift_y_arcsec)
        ...
        if np.abs(verbose) >= 3 and debug_ctx is not None and output_dir is not None:
            ...
            debug_ctx.ax.set_title(f"{self.node_name} [{self.node_id}]\nCorr={best_params['corr']:.4f}")
            debug_ctx.render_alignment_overlay(
                ...
                corrected_map=None,  # Don't show corrected in binned space
            )
            ...
            debug_ctx.render_comparison_animation(
                # ref_map=reference_binned if self.bin_kernel.value > 0 else reference_reprojected,
